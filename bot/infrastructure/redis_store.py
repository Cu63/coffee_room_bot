...
class RedisStore:
    ...
    async def mute_roulette_create(
        ...
        ttl = int(ends_at - time.time()) + 300  # с запасом
        await self._r.set(key, data, ex=max(ttl, 60))

    async def mute_roulette_get(self, chat_id: int) -> dict | None:
        key = f"{self._MUTE_ROULETTE}{chat_id}"
        raw = await self._r.get(key)
        if raw is None:
            return None
    ...
    async def mute_roulette_join(self, chat_id: int, user_id: int) -> bool:
    ...
    async def mute_roulette_delete(self, chat_id: int) -> dict | None:
    ...
    async def mute_roulette_count(self, chat_id: int) -> int:
        key = f"{self._MUTE_ROULETTE}{chat_id}"
        raw = await self._r.get(key)
        if raw is None:
            return 0
