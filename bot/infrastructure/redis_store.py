...
class RedisStore:
    ...
    _WG_RWORD_CD = "wg:rword_cd:"  # wg:rword_cd:{chat_id}

    async def wg_rword_cooldown_active(self, chat_id: int) -> int | None:
        """Проверить кулдаун /rword (на весь чат). Возвращает оставшееся время в секундах или None."""
        key = f"{self._WG_RWORD_CD}{chat_id}"
        ttl = await self._r.ttl(key)
        return ttl if ttl and ttl > 0 else None

    async def wg_rword_cooldown_set(self, chat_id: int, seconds: int) -> None:
        """Установить кулдаун /rword (на весь чат)."""
        key = f"{self._WG_RWORD_CD}{chat_id}"
        await self._r.set(key, "1", ex=seconds)

    _ANAGRAM_CD = "anagram:cd:"  # anagram:cd:{chat_id}

    async def anagram_cooldown_active(self, chat_id: int) -> int | None:
        """Проверить кулдаун /anagram (на весь чат). Возвращает оставшееся время в секундах или None."""
        key = f"{self._ANAGRAM_CD}{chat_id}"
        ttl = await self._r.ttl(key)
        return ttl if ttl and ttl > 0 else None

    async def anagram_cooldown_set(self, chat_id: int, seconds: int) -> None:
        """Установить кулдаун /anagram (на весь чат)."""
        key = f"{self._ANAGRAM_CD}{chat_id}"
        await self._r.set(key, "1", ex=seconds)
